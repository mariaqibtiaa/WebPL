...
if (empty($_SESSION['admin'])) {
    ...
    if (isset($_REQUEST['submit'])) {
        ...
        if (
            $_REQUEST['asal_surat'] == "" || $_REQUEST['no_surat'] == "" || $_REQUEST['isi'] == ""
            ...
            if (!preg_match("/^[a-zA-Z0-9.\/ -]*$/", $asal_surat)) {
                $_SESSION['easal_surat'] = 'Form Asal Surat hanya boleh mengandung karakter huruf, angka, spasi, titik(.), minus(-) dan garis miring(/)';
                echo '<script language="javascript">window.history.back();</script>';
            } else {
                            ...
                            if (!preg_match("/^[0-9.-]*$/", $tgl_surat)) {
                                ...
                                if (!preg_match("/^[a-zA-Z0-9.,()\/ -]*$/", $keterangan)) {
                                    ...
                                    if ($file != "") {
                                        ...
                                        if (in_array($eks, $ekstensi) == true) {
                                            if ($ukuran < 2300000) {
                                                ...
                                                if (!empty($file)) {
                                                    ...
                                                    $query = mysqli_query($config, "UPDATE tbl_surat_masuk SET asal_surat='$asal_surat',no_surat='$no_surat',isi='$isi',indeks='$indeks',tgl_surat='$tgl_surat',file='$nfile',keterangan='$keterangan',id_user='$id_user' WHERE id_surat='$id_surat'");
                                        ...
                                        $query = mysqli_query($config, "UPDATE tbl_surat_masuk SET asal_surat='$asal_surat',no_surat='$no_surat',isi='$isi',indeks='$indeks',tgl_surat='$tgl_surat',keterangan='$keterangan',id_user='$id_user' WHERE id_surat='$id_surat'");
        ...
        $query = mysqli_query($config, "SELECT id_surat, asal_surat, no_surat, isi, indeks, tgl_surat, file, keterangan, id_user FROM tbl_surat_masuk WHERE id_surat='$id_surat'");
        list($id_surat, $asal_surat, $no_surat, $isi, $indeks, $tgl_surat, $file, $keterangan, $id_user) = mysqli_fetch_array($query);
                ...
                <form class="col s12" method="POST" action="?page=tsm&act=edit" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        <div class="input-field col s12">
                            ...
                            <input id="asal_surat" type="text" class="validate" value="<?php echo $asal_surat; ?>" name="asal_surat" required>
                            <?php
